...
uint256 hashGenesisBlock("00000568b12b3cea1c4e6dc4035e7dd1e2148357ec5d36ffada1632b4b4c8db3"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // soundbit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "soundbit Signed Message:\n";
...
{
    // soundbit: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // soundbit: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // soundbit: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // soundbit: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, soundbit - DarkGravity v2, written by Evan Duffield - evan@soundbit.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, soundbit - DarkGravity v3, written by Evan Duffield - evan@soundbit.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // soundbit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // soundbit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Sound is a weapon! Spread it!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x67b860308b3a26c15ea225eb41b838eaea9bb3ac024f66b15d0916f729ac0777"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("soundbitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("soundbitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static soundbitMiner(CWallet *pwallet)
{
    printf("soundbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("soundbit-miner");
    ...
    try { loop {
        ...
        printf("Running soundbitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("soundbitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&soundbitMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - soundbit **********
    Copyright 2014, soundbit Developers
        eduffield - evan@soundbit.io
        InternetApe - kyle@soundbit.io
*/
